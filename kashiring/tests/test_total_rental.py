...
def calculate_expected_rental_price(pickup_datetime, return_datetime, base_price_per_day, rental_discount=None):
    """
...
@pytest.mark.parametrize("rental_payload", [
...
def test_rental_price(rental_payload, rental_discount, api_client, user, car_1, request):
    """
   ...
   # Получаем тестовые данные
    ...
    pickup_datetime, return_datetime = map(datetime.fromisoformat, 
                                        [rental_data["pickup_datetime"], rental_data["return_datetime"]])

    # Рассчитываем ожидаемую стоимость аренды
    expected_price = calculate_expected_rental_price(pickup_datetime, return_datetime, car_1.price_per_day, rental_discount)
    ...
    assert Decimal(response.data["total_price"]) == expected_price, (
