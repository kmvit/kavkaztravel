...
class CarViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        today = now()
        rented_cars = Rental.objects.filter(Q(return_datetime__gte=today)).values_list(
            "car_id", flat=True
        )
        ...
        if self.action == "retrieve":
            return queryset.prefetch_related(
                Prefetch(
                    ...
                    to_attr="features_list",
                ),
                Prefetch(
                    "images", queryset=CarImage.objects.all(), to_attr="images_list"
                ),
                Prefetch(
                    "discount_policy",
                ...
                Prefetch(
                    "options", queryset=CarOption.objects.all(), to_attr="options_list"
                ),
                Prefetch(
                    "equipments",
                    queryset=CarEquipment.objects.all(),
                    to_attr="equipments_list",
                ),
            )
        ...
        return queryset.prefetch_related(
            Prefetch(
                "images",
                queryset=CarImage.objects.only("image").order_by("id"),
                to_attr="first_image",
            ),
        ).only(
            "id",
