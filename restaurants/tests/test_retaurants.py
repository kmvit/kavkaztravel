...
def test_update_restaurant(authenticated_api_client, restaurant, child_region):
    ...
    assert initial_restaurant.restaurant_type.name == "Кавказская кухня"  # предположим, что тип ресторана такой
    assert initial_restaurant.services.count() == 8  # Услуги должны быть добавлены через фикстуру
    ...
    update_data = {
        ...
        "services": [
            {
                "name": "789",
                "description": "Обновленная услуга wi-fi"
            }
        ]
    }
    # Отправляем запрос на обновление
    response = authenticated_api_client.put(f"/api/v1/restaurants/meal/{restaurant.id}/", update_data, format='json')
    ...
    updated_services = updated_restaurant.services.all()  # Получаем все связанные услуги
    assert updated_services.count() == 1  # Убедитесь, что услуга была обновлена

    # Теперь проверяем атрибуты связанной услуги
    updated_service = updated_services.first()  # Получаем первую услугу (поскольку это one-to-many)
    assert updated_service.name == "789"  # Проверяем, что имя услуги обновилось
    assert updated_service.description == "Обновленная услуга wi-fi"  # Проверяем описание услуги
    ...
    assert updated_restaurant.description == "Лучшие блюда кавказской и европейской кухни"
...
def test_delete_restaurant(authenticated_api_client, restaurant):
    ...
    response = authenticated_api_client.delete(f"/api/v1/restaurants/meal/{restaurant_id}/")
    
    # Проверки на успешное удаление
    assert response.status_code == 204, f"Expected 204, but got {response.status_code}"  # Ожидаем статус 204 No Content
    
    # Проверяем, что ресторан был удален из базы данных
    assert not Restaurant.objects.filter(id=restaurant_id).exists(), "Restaurant was not deleted"
...
def test_restaurant_list_with_pagination_and_fields_check(
    ...
    for i in range(7):
        r_type = RestaurantType.objects.create(
            name=f"Тип ресторана {i+1}",
            description=f"Описание типа {i+1}"
        )
    ...
    response = authenticated_api_client.get("/api/v1/restaurants/meal/", {"page": 1}, format='json')
    ...
    assert response.data.get('count') == 8, "Total count of restaurants should be 8"
    
    # Проверяем пагинацию: 5 ресторанов на первой странице
    results = response.data.get('results', [])
    assert len(results) == 5, "Expected 5 restaurants on page 1"
    
    # Проверяем ссылки пагинации
    assert 'next' in response.data and response.data['next'] is not None, "'next' pagination link should be present"
    assert 'previous' in response.data and response.data['previous'] is None, "'previous' should be None on first page"
    ...
    assert rest_data['region'] == str(child_region), f"Restaurant 'region' should be {child_region}"

    assert 'short_description' in rest_data, "Restaurant 'short_description' should be present"
    assert rest_data['short_description'] == "Описание ресторана 1...", "Restaurant 'short_description' should match"

    assert 'restaurant_type' in rest_data, "Restaurant 'restaurant_type' should be present"
    assert rest_data['restaurant_type']['name'] == "Тип ресторана 1", "Restaurant type name should be 'Тип ресторана 1'"
    ...
    assert isinstance(main_image, str), "Main image should be a string (URL or file path)"
    ...
    assert rest_data['average_check'] == "1500.00", "Restaurant 'average_check' should be '1500.00'"

    # Проверяем пагинацию: вторая страница должна содержать 3 ресторана (8 - 5 = 3)
    response_page_2 = authenticated_api_client.get("/api/v1/restaurants/meal/", {"page": 2}, format='json')
    assert response_page_2.status_code == status.HTTP_200_OK, f"Expected 200, got {response_page_2.status_code}"
    results_page_2 = response_page_2.data.get('results', [])
    assert len(results_page_2) == 3, "Expected 3 restaurants on page 2"
...
@pytest.mark.django_db
def test_filter_by_region(authenticated_api_client, restaurants, child_region, additional_regions):
    """Тест для фильтрации ресторанов по региону."""
    # Фильтруем по региону "Москва"
    response = authenticated_api_client.get("/api/v1/restaurants/meal/", {"region": "Москва"}, format='json')
    assert response.status_code == status.HTTP_200_OK, f"Expected 200, got {response.status_code}"
    
    # Подсчитываем рестораны, относящиеся к Москве
    expected_count = sum(1 for restaurant in restaurants if restaurant.region.name == "Москва")
    print(expected_count)
    # Проверяем, что количество ресторанов в ответе совпадает с ожидаемым
    assert response.data['count'] == expected_count, f"Expected {expected_count} restaurants, got {response.data['count']}"
...
def test_filter_by_average_check(authenticated_api_client, restaurants):
    ...
    response = authenticated_api_client.get("/api/v1/restaurants/meal/", {"average_check_min": 2000}, format='json')
    assert response.status_code == status.HTTP_200_OK, f"Expected 200, got {response.status_code}"
    
    # Подсчитываем рестораны, где average_check >= 2000
    expected_count = sum(1 for restaurant in restaurants if restaurant.average_check >= 2000)
    print(expected_count)
    # Проверяем, что количество ресторанов в ответе совпадает с ожидаемым
    assert response.data['count'] == expected_count, f"Expected {expected_count} restaurants, got {response.data['count']}"
...
def test_filter_by_service(authenticated_api_client, restaurants, services):
    ...
    response = authenticated_api_client.get("/api/v1/restaurants/meal/", {"services": "Шашлык"}, format='json')
    assert response.status_code == status.HTTP_200_OK, f"Expected 200, got {response.status_code}"
    
    # Подсчитываем рестораны, которые имеют услугу "Шашлык"
    expected_count = sum(1 for restaurant in restaurants if "Шашлык" in [service.name for service in restaurant.services.all()])
    print(expected_count)
    # Проверяем, что количество ресторанов в ответе совпадает с ожидаемым
    assert response.data['count'] == expected_count, f"Expected {expected_count} restaurants, got {response.data['count']}"
...
def test_filter_by_restaurant_type(authenticated_api_client, restaurants):
    ...
    response = authenticated_api_client.get("/api/v1/restaurants/meal/", {"restaurant_type": "Кавказская кухня"}, format='json')
    assert response.status_code == status.HTTP_200_OK, f"Expected 200, got {response.status_code}"
    
    # Подсчитываем рестораны, которые имеют тип "Кавказская кухня"
    expected_count = sum(1 for restaurant in restaurants if restaurant.restaurant_type.name == "Кавказская кухня")
    print(expected_count)
    # Проверяем, что количество ресторанов в ответе совпадает с ожидаемым
    assert response.data['count'] == expected_count, f"Expected {expected_count} restaurants, got {response.data['count']}"
...
def test_filter_by_restaurant_type_and_service(authenticated_api_client, restaurants):
    ...
    response = authenticated_api_client.get("/api/v1/restaurants/meal/", {
        "restaurant_type": "Итальянская кухня",
        "services": "Паста"
    }, format='json')
    ...
    expected_count = sum(1 for restaurant in restaurants if 
                          restaurant.restaurant_type.name == "Итальянская кухня" and
                          "Паста" in [service.name for service in restaurant.services.all()])
...
@pytest.mark.django_db
def test_retrieve_restaurant(authenticated_api_client, restaurant, restaurant_image_1, services):
    """Тест проверяет, что метод retrieve возвращает ресторан с корректными полями."""
    
    response = authenticated_api_client.get(f"/api/v1/restaurants/meal/{restaurant.id}/", format='json')
    ...
    assert data["average_check"] == f"{restaurant.average_check:.2f}", "Средний чек должен совпадать и быть строкой"
    assert data["description"] == restaurant.description, "Описание ресторана должно совпадать"
    assert data["working_hours"] == restaurant.working_hours, "Часы работы должны совпадать"

    # Проверка объекта restaurant_type
    assert isinstance(data["restaurant_type"], dict), "restaurant_type должен быть объектом"
    assert data["restaurant_type"]["id"] == restaurant.restaurant_type.id, "ID типа ресторана должен совпадать"
    assert data["restaurant_type"]["name"] == restaurant.restaurant_type.name, "Название типа ресторана должно совпадать"
    assert data["restaurant_type"]["description"] == restaurant.restaurant_type.description, "Описание типа ресторана должно совпадать"

    # Проверка списка услуг
    assert isinstance(data["services"], list), "services должен быть списком"
    assert len(data["services"]) == restaurant.services.count(), "Количество услуг должно совпадать"
    for service in data["services"]:
        assert isinstance(service, dict), "Каждый элемент в services должен быть объектом"
        assert "id" in service and isinstance(service["id"], int), "service.id должен быть числом"
        assert "name" in service and isinstance(service["name"], str), "service.name должен быть строкой"
        assert isinstance(service["description"], (str, type(None)))
        assert any(s.id == service["id"] for s in restaurant.services.all()), "ID услуги должен быть в списке услуг ресторана"

    # Проверка списка изображений
    assert isinstance(data["images"], list), "images должен быть списком"
    assert len(data["images"]) == restaurant.images.count(), "Количество изображений должно совпадать"
    for image in data["images"]:
        assert isinstance(image, dict), "Каждый элемент в images должен быть объектом"
        assert "id" in image and isinstance(image["id"], int), "image.id должен быть числом"
        assert "image" in image and isinstance(image["image"], str) and image["image"].startswith("http"), "image.image должен быть URL-строкой"
        assert "restaurant" in image and image["restaurant"] == restaurant.id, "image.restaurant должен совпадать с ID ресторана"
...
def test_upload_restaurant_image(authenticated_api_client, restaurant):
    ...
    image = BytesIO()
    img = Image.new("RGB", (100, 100), color="red")  # Создаем изображение 100x100 пикселей
    img.save(image, "JPEG")
