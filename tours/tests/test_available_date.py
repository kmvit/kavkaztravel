...
def test_start_date_in_past(api_client, tour):
    ...
    assert "не может быть в прошлом" in response.json()["start_date"][0]  # Проверяем текст ошибки
...
def test_end_date_in_past(api_client, tour):
    ...
    assert "не может быть в прошлом" in response.json()["end_date"][0]  # Проверяем текст ошибки
